function [deltaG] = llt_solver2(vertices,collocation_points,b,nperfil,csi,vinf,u_n,u_a,mac,reynolds_info,airfoil_info,airfoil_data,aoa_eff,G,R,h,N,linear_check)
% V1.2
...
for i=1:N
    ...
    if strcmp(linear_check,'linear') == 1
        cl_alfa = llt_linear_solver(reynolds_info,airfoil_info,airfoil_data,b,nperfil,aoa_eff(i)*180/pi,i,'cl_alfa');
    else
        cl_alfa = llt_data_solver(reynolds_info,airfoil_info,airfoil_data,b,nperfil,aoa_eff(i)*180/pi,i,'cl_alfa');
    end
    cl_alfa = cl_alfa*180/pi;
    for j=1:N
