    ...
    for i in range(len_cp):
        ...
        for j in range(len_vp - 1):
            ...
            if ground_effect_check:
                ...
                v_ij_ge = get_induced_velocity_ground_effect(
                    cp_i, vp_jj, vp_j, mac_i, freestream_velocity[i], h, same_panel_check
                )
                v_ij += v_ij_ge
            v_ij_distr[i, j, :] = v_ij
...
def get_induced_velocity(
    ...
    if not np.isclose(
        ...
        bound_vortex_velocity = mac / (4 * np.pi) * ((ri1j_abs + ri2j_abs) * r12_cross_prod / bound_vortex_den)
...
def get_induced_velocity_ground_effect(
    ...
    if not same_panel_check:  # só adiciona essa parcela se a velocidade induzida não for em relação a si mesmo
        velocity_ij += mac / (4 * np.pi) * ((ri1j_abs + ri2j_abs) * r12_cross_prod / bound_vortex_den)
