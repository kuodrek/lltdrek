...
def get_span_y_distr(n, span_partition, distribution_type):
    ...
    vertice_points[0] = span_partition * 1 / 2 * (1 - np.cos(0))  # O primeiro ponto é 0
    for i in range(n):
        ...
        if distribution_type == "cosine":
            ...
            raise Exception("Invalid 'distribution_type' value. Expected 'cosine' or 'linear'.")

    span_y_distribution = {"vertice_points": vertice_points, "collocation_points": collocation_points}
...
def get_span_x_distr(cp_y_component, vp_y_component, chord_i, chord_ii, offset_i, offset_ii, span_partition):
    ...
    vp_x_component[0] = (
        0.25 * (chord_ii - chord_i) / span_partition * vp_y_component[0]
        + 0.25 * chord_i
        + (offset_ii - offset_i) / span_partition * vp_y_component[0]
        + offset_i
    )
    for i in range(n):
        vp_x_component[i + 1] = (
            0.25 * (chord_ii - chord_i) / span_partition * vp_y_component[i + 1]
            + 0.25 * chord_i
            + (offset_ii - offset_i) / span_partition * vp_y_component[i + 1]
            + offset_i
        )

        cp_x_component[i] = (
            0.25 * (chord_ii - chord_i) / span_partition * cp_y_component[i]
            + 0.25 * chord_i
            + (offset_ii - offset_i) / span_partition * cp_y_component[i]
            + offset_i
        )

    span_x_distribution = {"vertice_points": vp_x_component, "collocation_points": cp_x_component}
...
def get_span_z_distr(cp_y_component, vp_y_component, dihedral_partition):
    ...
    span_z_distribution = {"vertice_points": vp_z_component, "collocation_points": cp_z_component}
