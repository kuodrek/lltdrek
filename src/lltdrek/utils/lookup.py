...
def check_value_out_of_bounds(
    value: Union[int, float], lower_bound: Union[int, float], upper_bound: Union[int, float]
) -> bool:
...
def get_airfoil_data(
    cp_airfoil: list, cp_reynolds: float, aoa: float, airfoil_data: dict, cl_alpha_check: bool, show_logs: bool = True
) -> float:
    """
...
def get_linear_data_and_clmax(cp_airfoil: list, cp_reynolds: float, airfoil_data: dict, show_logs: bool = True) -> dict:
    """
    ...
    if len(cp_airfoil[1]) > 1:
        ...
        airfoil_root = cp_airfoil[1][0]
        lookup_data_root = cl_lookup(
            airfoil_data[airfoil_root], cp_reynolds, aoa, cl_alpha_check, linear_check, show_logs
        )
        airfoil_tip = cp_airfoil[1][1]
        lookup_data_tip = cl_lookup(
            airfoil_data[airfoil_tip], cp_reynolds, aoa, cl_alpha_check, linear_check, show_logs
        )

        lookup_data = {
            "cl_alpha": lookup_data_root["cl_alpha"] * (1 - merge_parameter)
            + lookup_data_tip["cl_alpha"] * merge_parameter,
...
def cl_lookup(
    ...
    if reynolds_number <= reynolds_list[0]:
        ...
        cl = aoa_list_lookup(cl_data, aoa) if cl_alpha_check is False else get_non_linear_cl_alpha(cl_data, aoa)
        ...
        for i in range(len(reynolds_list) - 1):
            ...
            if re_i <= reynolds_number <= re_ii:
                ...
                cl_i = (
                    aoa_list_lookup(cl_data_i, aoa)
                    if cl_alpha_check is False
                    else get_non_linear_cl_alpha(cl_data_i, aoa)
                )
                cl_ii = (
                    aoa_list_lookup(cl_data_ii, aoa)
                    if cl_alpha_check is False
