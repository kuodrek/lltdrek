...
class Wing:
    ...
    allowed_distribution_types = ["linear", "cosine"]
    ...
    def generate_mesh(self) -> None:
        ...
        self.cp_dsl = np.zeros(
            [self.N_panels, 3]
        )  # Vetor de comprimento ao longo da envergadura adimensional (dimensionless spanwise length vector)
        ...
        for i, span_partition in enumerate(self.spans):
            ...
            sweep_partition = (
                np.arctan((0.25 * chord_ii - 0.25 * chord_i + offset_i) / span_partition)
                if self.sweep_check is True
                else 0
            )
            ...
            span_x_distr = geo.get_span_x_distr(
                cp_y_component, vp_y_component, chord_i, chord_ii, offset_i, offset_ii, span_partition
            )
            ...
            for j, _ in enumerate(cp_y_component):
                ...
                for k in range(3):
                    self.cp_lengths[idx_n + j][k] = (
                        self.vertice_points[idx_n + j + 1][k] - self.vertice_points[idx_n + j][k]
                    )
        ...
        for i, span_partition in enumerate(self.spans):
            ...
            for j in range(self.span_panel_numbers[i]):
                if airfoil_i == airfoil_ii:
                    ...
                    merge_parameter = (
                        self.collocation_points[idx_n + j][1] - self.vertice_points[idx_n][1]
                    ) / span_partition
