...
wingpool = WingPool(wing_list=[asa, asa_2], flight_condition=flight_condition)

simulation = Simulation(wing_pool=wingpool, damping_factor=1, max_iter=150, max_residual=1e-3, linear_check=True)
...
for i, G in enumerate(G_solution_list):
    ...
    coefs_single = llt_coeficientes.get_wing_coefficients(
        wingpool, G, aoa_index=i, S_ref=asa.total_area * 2, c_ref=asa.MAC
    )
