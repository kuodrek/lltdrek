...
flight_condition = FlightCondition(V_inf=45.48, nu=6.39e-6, rho=2.83, aoa=aoa_list_1, ground_effect_check=False)

asa.setup_airfoil_data(flight_condition, airfoils_data_dict)

wingpool = WingPool(wing_list=[asa], flight_condition=flight_condition)
...
for i, _ in enumerate(G_solution_list):
    coefs = llt_coeficientes.get_global_coefficients(
        wingpool, G_solution_list[i], aoa_index=i, S_ref=asa.total_area * 2, c_ref=asa.MAC
    )
