...
def generate_mesh(Wing: models.Wing):
    ...
    if sum(b_partitions) == N_panels - 1:
        b_partitions[0] += 1
    ...
    for i in range(N_partitions):
        ...
        if Wing.panels_distr == "cosine":
            ...
            aspace2 = np.arange(
                np.pi / (2 * b_partitions[i]),
                np.pi / b_partitions[i],
                np.pi - np.pi / (2 * b_partitions[i]),
            )
            ...
            raise ValueError(
                "Valor inválido em Wing.panels_distr: Esperava-se 'linear' ou 'cosine'"
            )
